...
while running:
    try:
        ...
        # Capturar KeyboardInterrupt (Ctrl+C) para salir de manera limpia
        running = False

# Asegúrate de llamar a pygame.quit() al salir del bucle
pygame.quit()


def main():
    global font, run  # Añadido 'run' a la lista de variables globales
    pygame.init()
    pygame.display.set_caption("Tetris con PyGame")
    screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
    run = True  # Inicialización de 'run'
    paused = False
    game_over = False
    # Create background.
    ...
    try:
        ...
        # If the font file is not available or there's an error, use the default font.
            ...
            if event.type == pygame.QUIT:
                ...
                if not paused and not game_over:
                    if event.key in MOVEMENT_KEYS:
                        ...
                        blocks.current_block.rotate_clockwise(blocks)
                    elif event.key == pygame.K_DOWN:
                        blocks.current_block.rotate_counterclockwise(blocks)
